...
export default async function Relatorio({params}: {params: Promise<{turma: string}>}) {
    const {turma} = await params;
    const alunos = await getAlunosPorTurma(turma.toUpperCase());
    ...
    <div className="flex flex-col items-center justify-center mx-auto  p-4">
      ...
      <div className="flex flex-col items-center justify-center w-max-2xl w-min-[300px] b">
        <div className="flex flex-col justify-center w-full  gap-4">
            <div key={turma}>
              <h2 className="text-2xl font-bold">Turma {turma.toUpperCase()}</h2>
